...
mermod_R_error <- tryCatch({
  compute_lai_2spa_inputs(fit, split_dat, id_df, R_list = diag_R_list)
  FALSE
...
fit_nlme <- if (requireNamespace("nlme", quietly = TRUE)) {
  nlme::lme(
    fixed = y ~ z,
    random = ~1 + z | id,
    data = sim_dat,
...
if (!is.null(fit_nlme)) {
  out_ar_R <- compute_lai_2spa_inputs(fit_nlme, split_dat, id_df, R_list = ar_R_list)

  first_id <- id_df$id[[1]]
  df_1 <- split_dat[[first_id]]
  Z_1 <- cbind(1, df_1$z)
  G_hat <- as_plain_vcov_matrix(nlme::getVarCov(fit_nlme, type = "random.effects"))
  beta_hat <- nlme::fixef(fit_nlme)
  ...
  stopifnot(
    ...
    isTRUE(all.equal(unname(row_1$lambda11), unname(expected_lambda_1[1, 1]), tolerance = 1e-8)),
    isTRUE(all.equal(unname(row_1$lambda12), unname(expected_lambda_1[1, 2]), tolerance = 1e-8)),
    isTRUE(all.equal(unname(row_1$lambda21), unname(expected_lambda_1[2, 1]), tolerance = 1e-8)),
    isTRUE(all.equal(unname(row_1$lambda22), unname(expected_lambda_1[2, 2]), tolerance = 1e-8)),
